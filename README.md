...
Keeper APY collection uses SDK-first lookups with API and static fallbacks because current SDK versions do not expose a stable cross-provider APY surface. The frontend reads state directly from Anchor accounts, surfaces browser-installed wallets like Phantom and Solflare as primary connect targets, separates app-based wallet flows when they are not usable in the current runtime, and falls back gracefully from Helius to basic Solana RPC history when enhanced indexing is unavailable. The dashboard now exposes a live network badge, copy-to-clipboard actions for operational values, calmer network-state messaging when a vault is unavailable, confirmation modals before state-changing actions, a short changelog page, and a visible frontend version marker so operators can tell exactly what environment and build they are looking at.
